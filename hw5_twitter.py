...
CACHE_TWITTER = "twitter_cache.json"

## check if data is fetched from the cache
try:
    cache_file = open(CACHE_TWITTER, 'r')
...
def params_unique_combination(base_url, params_dict):
    ...
    for key in alphabetized_keys:
        ...
        unique_ident = base_url + "_".join(res)
    return unique_ident
...
# f = open(resp_file, "w")
# resp_str = json.dumps(results_dict)
# f.write (resp_str)
# f.close()
...
for tweet in results_dict:
    ...
    # iterate through the tokenized words and filter out the stopwords
    for word in tokens:
